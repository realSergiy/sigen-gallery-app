...
export function GET() {
  ...
  if (REQUIRE_ENV_VARS) {
    ...
    url.searchParams.set('env', ['NEXT_PUBLIC_SITE_TITLE'].join(','));
  }
  url.searchParams.set('teamCreateStatus', 'hidden');
  url.searchParams.set(
    'stores',
    JSON.stringify([{ type: 'postgres' }, { type: 'blob' }]),
  );
