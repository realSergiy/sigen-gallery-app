...
export default function ResponsiveDate({
  ...
  return (
    <span title={title} className={clsx(className, 'uppercase')}>
      {/* Small */}
      <span className="xs:hidden" aria-hidden>
        {formatDate(date, 'short')}
      </span>
      {/* Medium */}
      <span className="hidden xs:inline-block sm:hidden" aria-hidden>
        {formatDate(date, 'medium')}
      </span>
      {/* Large */}
      <span className="hidden sm:inline-block">{formatDate(date)}</span>
    </span>
  );
}
